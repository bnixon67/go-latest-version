...
func TestDownloadFileWithProgressAndChecksum(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Create a unique temp file for each test case
			var destPath string
			if tc.invalidFilepath {
				destPath = "/invalid/path/to/file.txt"
			} else {
			...
			size, checksum, err := DownloadFileWithProgressAndChecksum(tc.url, destPath, tc.expectedSize, sha256.New())
